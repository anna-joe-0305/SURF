...
file_objectlocations = sprintf('%sSURF_object_locations.txt', folder);
...
%%adjust for correct calculation of flow rate
dt = 0.0005; %delay between frames (s)
canal_depth = 8*10^-6;
meter_per_pixel = 0.55*10^-6; %Photron 20x: 90Px = 50mum
...
%one for each trace
for n = 1:number_filtered_traces
    filename_trace = sprintf('SURF_trace %d.txt', n);
    TracesDataFileName = [folder filename_trace];
    fid = fopen(TracesDataFileName,'w');
    if fid ~= -1
        
        fprintf(fid,'%s\r\n', '===...===');
        fprintf(fid,'%s\t%s%s\r\n','results SURF', folder , video_filename);
        fprintf(fid,'%s\t%s\r\n','total traces: ', num2str(number_filtered_traces) );
        fprintf(fid,'%s\t%s\r\n','flow rate (mul/h): ', num2str(flowrate) );
        fprintf(fid,'%s\r\n', '===...===');
